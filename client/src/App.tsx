...
function App() {
  const { currentScene, currentIndex, progress, isPaused, goToScene, togglePause } =
    useSceneRotation();
  ...
  return (
    <div className="min-h-screen bg-[#080C10] flex flex-col overflow-hidden">
      ...
      <div
        key={currentScene.id}
        className="scene-enter flex-1 flex items-center justify-center p-8"
      >
        <div className="text-center">
          <div className="font-mono text-xs text-white/20 uppercase tracking-widest mb-4">
            Now Showing
          </div>
          <h1
            className={`text-6xl font-bold mb-4 ${currentScene.category === 'brand' ? 'text-[#39FF14]' : 'text-[#00FFFF]'}`}
          >
            {currentScene.label}
          </h1>
          ...
          <p className="font-mono text-white/20 text-xs mt-2">
            {isPaused ? 'Paused' : `${Math.round(progress)}% complete`}
          </p>
        </div>
      </div>
